...
from app.orchestrator.models import ExecutionContext, AgentResult
from app.orchestrator.handlers.response_handler import ResponseHandler
from app.orchestrator.conversation.manager import ConversationManager
...
- **ALWAYS recommend charts for**: trend analysis, time series, comparisons, top/bottom rankings, growth analysis
- **Chart types to suggest**:
  - Line charts: for trends over time, growth patterns, timeline data
  - Bar charts: for category comparisons, rankings, top performers
  - Pie charts: for percentage breakdowns, market share, distributions  
  - Metrics: for single key numbers, totals, KPIs
- **Enhanced chart specifications**: Include detailed axis labels, titles, color schemes, and data formatting
...
"""
    ...
    async def _handle_execution(self, context: ExecutionContext, intent: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Debug: Print the original dict results
        print(f"[SimpleOrchestrator] Original dict results: {len(results)} items")
        for i, result in enumerate(results):
            print(f"[SimpleOrchestrator] Dict result {i}: success={result.get('success')}, metadata={result.get('metadata')}")
        
        # Convert dict results to AgentResult objects for the response handler
        agent_results = [self._convert_dict_to_agent_result(result) for result in results]
        print(f"[SimpleOrchestrator] Converted {len(results)} dict results to AgentResult objects")
        
        # Debug: Print the converted results
        for i, agent_result in enumerate(agent_results):
            print(f"[SimpleOrchestrator] AgentResult {i}: success={agent_result.success}, metadata={agent_result.metadata}")
        
        # Use the ResponseHandler's proper execution complete method
        return await self.response_handler.handle_execution_complete(
            context, intent, agent_results, strategy
        )
    ...
    def _convert_dict_to_agent_result(self, result_dict: Dict[str, Any]) -> AgentResult:
        """Convert dictionary result to AgentResult object"""
        return AgentResult(
            agent_id=result_dict.get("agent_id", ""),
            agent_name=result_dict.get("agent_name", "Unknown Agent"),
            success=result_dict.get("success", False),
            data=result_dict.get("data"),
            error=result_dict.get("error"),
            row_count=result_dict.get("row_count", 0),
            query=result_dict.get("query"),
            metadata=result_dict.get("metadata", {"execution_type": "data" if "row_count" in result_dict else "application"}),
    ...
    async def _execute_agent(self, agent_id: str, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if agent_type == "data_agent":
                ...
                print(f"[SimpleOrchestrator] Using DataAgentExecutor for data agent {agent_id}")
                ...
                # Debug: Print the actual DataAgentExecutor result
                print(f"[SimpleOrchestrator] DataAgentExecutor result: success={result.success}, error={result.error}")
                print(f"[SimpleOrchestrator] DataAgentExecutor data length: {len(result.data) if result.data else 0}")
                print(f"[SimpleOrchestrator] DataAgentExecutor result type: {type(result)}")
                print(f"[SimpleOrchestrator] About to check result.success: {result.success}")
                
                # Convert AgentResult to dict format expected by the orchestrator
                try:
                    if result.success:
                        dict_result = {
                            "agent_id": agent_id,
                            ...
                            "query": result.query,
                            "metadata": {"execution_type": "data"},
                            "visualization": result.visualization
                        }
                        print(f"[SimpleOrchestrator] Created SUCCESS dict result: success={dict_result.get('success')}")
                        return dict_result
                    else:
                        dict_result = {
                            "agent_id": agent_id,
                        ...
                        print(f"[SimpleOrchestrator] Created FAILURE dict result: success={dict_result.get('success')}")
                        return dict_result
                except Exception as conversion_error:
                    print(f"[SimpleOrchestrator] ERROR during result conversion: {conversion_error}")
                    print(f"[SimpleOrchestrator] result.data type: {type(result.data)}")
                    print(f"[SimpleOrchestrator] result attributes: {dir(result)}")
                    return {
                        "agent_id": agent_id,
                        "agent_name": agent.get("name"),
                        "success": False,
                        "error": f"Result conversion error: {str(conversion_error)}",
                        "metadata": {"execution_type": "data"}
                    }
                ...
                return {
                    ...
                    "metadata": {"execution_type": "application"}
                }
